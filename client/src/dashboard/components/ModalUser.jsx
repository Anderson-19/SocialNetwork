...
export const ModalUser = ({ userData }) => {

    const { user, setUser } = useUserStore(state => state);
    const { isSideModalEditUser, closeModalEditUser } = useUIStore(state => state);
    ...
    return (
        <div className={`${isSideModalEditUser ? 'fixed flex justify-center items-center inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full transition-all' : 'hidden'}`}>
            <div className="border border-blue-500 modal-container bg-white w-4/12 max-[770px]:w-8/12 md:max-w-11/12 mx-auto rounded-xl shadow-lg z-50 overflow-y-auto">
